...
/**
 * Digital Chicken Labs Octoquad localizer
 *
 * @author Dylan B. - 18597 RoboClovers - Delta
 */
public class Octoquad extends BaseLocalizer<Octoquad.Constants> {
    private final Driver octoquad;
    private final Driver.LocalizerDataBlock localizer = new Driver.LocalizerDataBlock();

    public enum EncoderDirection {FORWARD, REVERSED}

    public Octoquad(Constants constants, HardwareMap hardwareMap) {
        super(constants);

        octoquad = hardwareMap.get(Driver.class, constants.name);
        octoquad.setEncoderDirection(constants.xPort, constants.xPodDirection);
        octoquad.setEncoderDirection(constants.yPort, constants.yPodDirection);
        octoquad.setLocalizerPortX(constants.xPort);
        octoquad.setLocalizerPortY(constants.yPort);
        octoquad.setLocalizerCountsPerMM_X((float) constants.encoderResolution.get(DistUnit.MM));
        octoquad.setLocalizerCountsPerMM_Y((float) constants.encoderResolution.get(DistUnit.MM));
        octoquad.setLocalizerTcpOffsetMM_X((float) constants.offsets.getX(DistUnit.MM));
        octoquad.setLocalizerTcpOffsetMM_Y((float) constants.offsets.getY(DistUnit.MM));
        octoquad.setLocalizerImuHeadingScalar((float) constants.angularScalar);
        octoquad.resetLocalizerAndCalibrateIMU();

        while (octoquad.getLocalizerStatus() != Driver.Status.RUNNING) {
            try {
    ...
    public static class Constants implements BaseLocalizerConstants<Constants> {
        ...
        public Octoquad build(HardwareMap hardwareMap) {
            if (name == null) {
                throw new IllegalArgumentException(
                        "Octoquad name is not set in the localizer constants."
                );
            }
            return new Octoquad(this, hardwareMap);
        }

        /** Sets the name of the Octoquad in the hardware map. */
        public Constants setName(String name) {
            this.name = name;
            return this;
        }

        /** Sets the port numbers of the X and Y encoders of the Octoquad. */
        public Constants setPorts(int xPort, int yPort) {
            this.xPort = xPort;
            this.yPort = yPort;
            return this;
        }

        /** Sets the X and Y pod offsets of the Octoquad. */
        ...
        /** Sets the direction of the X and Y encoders of the Octoquad. */
        ...
        /** Sets the encoder resolution of the Octoquad pods in ticks per unit. */
        public Constants setEncoderResolution(Dist encoderResolution) {
            this.encoderResolution = encoderResolution;
            return this;
        }

        /** Sets the angular scalar of the Octoquad. It is not recommended to change this values. */
    ...
    // TODO: Uncook this and make it nicer
    ...
    public static class Driver extends I2cDeviceSynchDevice<I2cDeviceSynchSimple> {
        ...
        /** The symbol for microseconds is μs, but is sometimes simplified to us. */
        ...
        enum RegisterType {
            ...
            RegisterType(int length) { this.length = length; }
        }
        ...
        public static class FirmwareVersion {
            ...
            @NonNull
            @Override
            public String toString() { return String.format(Locale.US, "%d.%d.%d", maj, min, eng); }
        }
        ...
        public enum ChannelBankConfig {
            ...
            /**
             * Bank 1 (channels 0-3) is configured for Quadrature input; Bank 2 (channels 4-7) is
             * configured for pulse width input.
             */
            BANK1_QUADRATURE_BANK2_PULSE_WIDTH(2);

            private final byte bVal;

            ChannelBankConfig(int bVal) { this.bVal = (byte) bVal; }
        }

        /**
         * Enum representing the status of the absolute localizer algorithm You can poll this to
         * determine when IMU calibration has finished.
         */
        public enum Status {
            INVALID(0),
            NOT_INITIALIZED(1),
        ...
        /**
         * The absolute localizer automagically selects the appropriate axis on the IMU to use for
         * yaw, regardless of physical mounting orientation. (However, you must mount in one of the
         * 6 axis-orthogonal orientations). You can poll this status to determine which axis was
         * chosen.
         */
        public enum LocalizerYawAxis {
            ...
            LocalizerYawAxis(int code) { this.code = code; }
        }

        /**
         * A data block holding all encoder data; this block may be bulk read in one I2C operation.
         * You should check if the CRC is OK before using the data.
         */
        public static class EncoderDataBlock {
            ...
            /**
             * Check whether it is likely that this data is valid by checking if the CRC on the
             * returned is bad. For example, if there is an I2C bus stall or bit flip, you could
             * avoid acting on corrupted data.
             *
             * @return whether it is likely that this data is valid
             */
            public boolean isDataValid() { return crcOk; }
        }
        ...
        public enum I2cRecoveryMode {
            ...
            /**
             * The OctoQuad will reset its I2C peripheral if 50ms elapses between byte transmissions
             * or between bytes and start/stop conditions
             */
            MODE_1_PERIPH_RST_ON_FRAME_ERR(1),

            /**
             * Mode 1 actions + the OctoQuad will toggle the clock line briefly, once, after 1500ms
            ...
            I2cRecoveryMode(int bVal) { this.bVal = (byte) bVal; }
        }
        ...
        @Override
        public String getDeviceName() { return "Octoquad"; }

        /** @return the value in the CHIP_ID register of the OctoQuad */
        public byte getChipId() { return readRegister(Register.CHIP_ID)[0]; }
        ...
        public FirmwareVersion getFirmwareVersion() {
            byte[] fw = readContiguousRegisters(Register.FIRMWARE_VERSION_MAJOR,
                    Register.FIRMWARE_VERSION_ENGINEERING);
        ...
        /**
         * Set the direction for a single encoder This has basically the same effect as negating
         * pos/vel in user code, except for when using the absolute localizer, in which case this
         * must be used to inform the firmware of the tracking wheel directions. This parameter will
         ...
         */
        public void setEncoderDirection(int idx, EncoderDirection direction) {
            verifyInitialization();
        ...
        /**
         * Reads all encoder data from the OctoQuad, writing the data into an existing
         * {@link EncoderDataBlock} object. The previous values are destroyed.
         *
        ...
        /**
         * Configure the minimum/maximum pulse width reported by an absolute encoder which is
         * connected to a given channel, to allow the ability to provide accurate velocity data.
         * These parameters will NOT be retained across power cycles, unless you call
         * {@link #saveParametersToFlash()} ()}
         *
        ...
        /**
         * Queries the OctoQuad to determine the currently set minimum/maxiumum pulse width for an
         * encoder channel, to allow sane velocity data.
         *
         * @param idx the channel in question
        ...
        /**
         * Configure whether in PWM mode, a channel will report the raw PWM length in microseconds,
         * or whether it will perform "wrap tracking" for use with an absolute encoder to turn the
         * absolute position into a continuous value.
         * <p>
         * This is useful if you want your absolute encoder to track position across multiple
         * rotations. NB: in order to get sane data, you MUST set the channel min/max pulse width
         * parameter. Do not assume these values are the same for each encoder, even if they are
         * from the same production run! REV Through Bore encoders have been observed to vary +/-
         * 10uS from the spec'd values. You need to actually test each encoder manually by turning
         * it very slowly until it gets to the wrap around point, and making note of the max/min
         * values that it flips between, and then program those values into the respective channel's
         * min/max pulse width parameter.
         * <p>
         * Note that when using a PWM channel in "wrap tracking" mode, issuing a reset to that
         * channel does NOT clear the position register to zero. Rather, it zeros the "accumulator"
         * (i.e. the number of "wraps") such that immediately after issuing a reset command, the
         * position register will contain the raw pulse width in microseconds. This behavior is
         * chosen because it maintains the ability of the absolute encoder to actually report its
         * absolute position. If resetting the channel were to really zero out the reported position
         * register, using an absolute encoder would not behave any differently than using a
         * relative encoder.
         *
         * @param idx the channel in question
        ...
        /**
         * Set the scalar for converting encoder counts on the X port to millimeters of travel You
         * SHOULD NOT calculate this - this is a real world not a theoretical one - you should
         ...
         *
         * @param ticksPerMM_x scalar for converting encoder counts on the X port to
        ...
        /**
         * Set the scalar for converting encoder counts on the Y port to millimeters of travel You
         * SHOULD NOT calculate this - this is a real world not a theoretical one - you should
         * measure this by pushing your robot N millimeters and dividing the counts by N.
         * <p>
         * NOTE: this will not take effect until a call to {@link #resetLocalizerAndCalibrateIMU()}
         *
         * @param ticksPerMM_y scalar for converting encoder counts on the Y port to
         *         millimeters of travel
         */
        ...
        /**
         * The real TCP (Tracking Center Point) of your robot is the point on the robot, where, if
         * the robot rotates about that point, neither the X nor Y tracking wheels will rotate. This
         * point can be determined by drawing imaginary lines parallel to, and through the middle
         * of, your tracking wheels, and finding where those lines intersect.
         * <p>
         * Without setting any TCP offset, if the robot rotates, the reported XY vales from the
         * localizer will change during rotation, unless the rotation is about the real TCP. Most
         * users will find this behavior rather unhelpful, since usually a robot will robot about
         * its geometric center, and not about the real TCP.
         * <p>
         * You can move the location of the localizer's "virtual" TCP away from the true location
         * (e.g. to the center of your robot) by applying an offset. This value does not need to be
         * extremely accurate since it will not affect the accuracy or repeatability of the
         * localizer algorithm.
         * <p>
         * NOTE: this will not take effect until a call to {@link #resetLocalizerAndCalibrateIMU()}
         *
         ...
         * <p>
         * Without setting any TCP offset, if the robot rotates, the reported XY vales from the
         * localizer will change during the rotation, unless the rotation is about the real TCP.
         * Most users will find this behavior rather unhelpful, since usually a robot will robot
         * about its geometric center, and not about the real TCP.
         * <p>
         * You can move the location of the localizer's "virtual" TCP away from the true location
         * (e.g. to the center of your robot) by applying an offset. This value does not need to be
         * extremely accurate since it will not affect the accuracy or repeatability of the
        ...
        /**
         * Set a scale factor to apply to the IMU heading to improve accuracy The recommended way to
         * tune this is to use the heading scalar tuning OpMode.
         * <p>
         * NOTE: this will not take effect until a call to {@link #resetLocalizerAndCalibrateIMU()}
         *
         * @param headingScalar scale factor to apply to the IMU heading to improve
         *         accuracy
         */
         ...
         * NOTE: this will not take effect until a call to {@link #resetLocalizerAndCalibrateIMU()}
         *
         * @param port the port index to be used by the absolute localizer routine for
         *         measuring X movement
         */
        ...
        /**
         * Set the period of translational velocity calculation. Longer periods give higher
         * resolution with more latency, shorter periods give lower resolution with less latency.
         * <p>
         * NOTE: this will not take effect until a call to {@link #resetLocalizerAndCalibrateIMU()}
         *
         * @param ms 1-255ms
        ...
        /**
         * Reset the localizer pose to (0,0,0) and recalibrate the IMU. Poll
         * {@link #getLocalizerStatus()} for {@link Status#RUNNING} to determine when the
         * reset is complete.
         */
        ...
        public static class LocalizerDataBlock {
            ...
            public boolean isPoseDataValid() {
                return crcOk && localizerStatus == Status.RUNNING;
            }
        }
        ...
        private void unpackLocalizerData(ByteBuffer buf, LocalizerDataBlock out) {
            ...
            // Prevent crash if value out of expected range
            if (localizerStatusCode < Status.values().length) {
                out.localizerStatus = Status.values()[localizerStatusCode];
            } else {
                out.localizerStatus = Status.INVALID;
            }
        ...
        /**
         * Bulk read all localizer data in one operation for maximum efficiency, writing the data
         * into an existing {@link LocalizerDataBlock} object. The previous values are destroyed.
         *
         ...
         *
         * @param localizerOut the {@link LocalizerDataBlock} object to fill with new
         *         localizer data
         * @param encoderOut the {@link EncoderDataBlock} object to fill with new encoder
         *         data
         */
        ...
        /**
         * "Teleport" the localizer to a new location. This may be useful, for instance, for
         * updating your position based on vision targeting.
         *
         * @param posX_mm x position in millimeters
         * @param posY_mm y position in millimeters
         * @param heading_rad heading in radians
         */
        public void setLocalizerPose(double posX_mm, double posY_mm, float heading_rad) {
            verifyInitialization();

            ByteBuffer buf = ByteBuffer.allocate(6);
            buf.order(OCTOQUAD_ENDIAN);

            long roundedX = Math.round(posX_mm);
            long roundedY = Math.round(posY_mm);

            short safeX = (short) Math.max(Short.MIN_VALUE, Math.min(Short.MAX_VALUE, roundedX));
            short safeY = (short) Math.max(Short.MIN_VALUE, Math.min(Short.MAX_VALUE, roundedY));
        ...
        /**
         * Teleport the localizer heading to a new orientation. This may be useful, for instance,
         * for updating heading based on vision targeting.
         *
        ...
        /**
         * Configures the OctoQuad to use the specified I2C recovery mode. This parameter will NOT
         * be retained across power cycles, unless you call {@link #saveParametersToFlash()} ()}
         *
        ...
        private short calc_crc16_profibus(byte[] data, int len) {
        ...
        private short calc_crc16_profibus(byte[] data) {
